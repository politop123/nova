...
- Commit reminder cancellation/rescheduling and the successful audit result in one transaction, deduplicate by user/request key, and reject stale reminder versions. Old queued jobs must skip delivery after rescheduling. If the queue fails after the new time is saved, explicitly tell the user delivery is not confirmed.
