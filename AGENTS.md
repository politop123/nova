...
- NOVA can answer read-only operational health questions about itself. Build these answers from real server-side checks for API, PostgreSQL, Redis, worker visibility, reminders, Telegram, OpenAI configuration, and Git/deploy state; do not infer a healthy state when a component cannot be checked.
