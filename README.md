...
Git/deploy awareness is configured with `NOVA_GIT_REPOSITORY`, `NOVA_GIT_BRANCH`, optional server-side `NOVA_GITHUB_TOKEN`, and `IMAGE_TAG`/`NOVA_DEPLOYED_COMMIT_SHA`. The API exposes `/api/v1/devops/git/status`, and NOVA can answer chat questions such as “Який останній commit?” or “Чи задеплоївся вже цей commit?”. `/api/v1/status` returns a broader operational snapshot for questions such as “NOVA, що з тобою?”, including a real worker heartbeat when the worker is running.
